...
seg-velocity                                                               ;; seg-velocity is the velocity in each segment
...
to setup                                                                   ;; this is what happens when the setup button on the interface is pushed, it sets up the model
 ...
 set-velocity
  ...
  [
    ...
    setxy 0 19                                                             ;; set the x,y coordinates of the 2 new fish to (0,19) from bottom left origin
...
to go                                                                      ;; initiate go sequence of events with the "go" button on the user interface 
  ...
  ask turtles [ do-migration ]                                             ;; make the salmon move upstream to the upstream segment with the lowest velocity (if lower velocity exists upstream)
                                                                           ;; "do-migration" is not working perfectly because I think I am getting patch variables and turtle variables mixed up
end

;------------------------------------------------------------------------------------------------------

to create-river                                                            ;; create the river; downstream/ocean is at left (x-coordinate 0) and upstream is at right
ask patches with [pycor > 15 and pycor < 25] [set pcolor blue]             ;; all patches with a y-coordinate > 15 and < 25 turn blue to indicate the river
...
ask patches with [pcolor = blue] [set seg-velocity random 6]               ;; all patches with a y-coordinate > 15 and < 25 take random integer value under 6 (cubic feet per second) for seg-velocity

ask patches with [pcolor = black] [set seg-velocity 30]                    ;; to keep the fish in the river, set all non-river patch seg-velocity to a high number (I couldn't figure out how to find the 
                                                                           ;; max velocity and then assign the area outside of the the river it to that value +1)
end

;------------------------------------------------------------------------------------------------------

to do-migration                                                            ;; make the salmon move upstream to the upstream segment with the lowest velocity (if lower velocity exists upstream)
